...
/**
 * The CDI <b>managed</b> bean for EJB checks.
 */
@Named
@RequestScoped
public class ChecksManagedBean {
    /*-
    As of JSF 2.3, '@ManagedBean' (bean managed by JSF container) is being phased out.
    The recommended approach is '@Named' (CDI bean managed by the application server).
    */
    ...
    private void compareBeansFromEjb() {
        ...
        String message = ("Compare two beans from '@EJB' injection: no-interface view[%b], " +
                          "stateless local view[%b], stateless remote view[%b], singleton remote view[%b]").formatted(
        ...
        try {
            ...
            logger.severe("compareBeansFromEjb(): exception[%s]".formatted(e.getMessage()));
            return;
        }
        message = ("Compare bean from lookup vs bean from '@EJB' injection: " +
        ...
        message = "Compare two beans from '@EJB' injection: stateful remote view[%b]".formatted(
    ...
    private void compareBeansFromInject() {
        ...
        final String message = ("Compare two beans from '@Inject' injection: " +
    ...
    private void compareBeans() {
        ...
        final String message = ("Compare bean from '@EJB' injection vs bean from '@Inject' injection: " +
                            "no-interface view[%b], stateless local view[%b]").formatted(
